...
The gateway applies per-IP rate limits, a bounded FIFO queue that dispatches
only one request at a time to Glyph's serial harness, per-dispatch deadlines,
a circuit breaker, bounded output, and curated fallbacks. The queue admits four
requests by default and can be tuned with `MAX_QUEUE_DEPTH`. Run `npm test` and
